...
- **Settled with the maintainer (fifth commit):** the viewer's `source_sha256` after a
  ...
  `just check` were.
- **Settled with the maintainer (sixth commit):** the digest above was advisory. An
  adversarial review of the branch found that `check_assets.py` whitelisted
  `source_sha256` and `patched` by name and never read them, and that `write` carried a
  stale `source_sha256` forward silently, so a typo, a stale value after a rebuild or an
  arbitrary digest passed `just check`. The review proposed reversing the three rewrites
  in the checker; declined, because `patched` is prose by design. Decided instead: the
  unpatched digest is already committed — `viewer.py` writes it as `sha256` in
  `qa/viewer-package.json`, a listed asset regenerated by every rebuild — so `write`
  reads the viewer's `source_sha256` from that record and `check` refuses a mismatch,
  and both optional fields are checked for shape (64 lowercase hex, unequal to
  `sha256`, a non-empty list of non-empty strings, never one without the other). Edits
  outside this ticket's table, each authorised by the maintainer: `scripts/check_assets.py`
  (S00, final) gains the checks and their self-test cases; `scripts/rebuild_model.sh`
  asserts its pre-rewrite digest equals the record and no longer asks for a copy;
  CONVENTIONS.md §4, the `Justfile` recipe comment and the `asset-change` skill say so.
  `rebuild-the-model.md` says so in its recipe and its after-run steps. Verified: the
  self-test's new cases, `check` and `write` on the real tree with an empty manifest
  diff, a flipped digit in the committed `source_sha256` refused by name, shellcheck, the
  usage path and `just check`. The rebuild success path still needs Blender and was not run.

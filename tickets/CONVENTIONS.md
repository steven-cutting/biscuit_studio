...
| `pyproject.toml` `[tool.typos.files] extend-exclude` | the six `assets/models/biscuit/<dir>/` paths, `"assets/illustrations/"` and `"static/pose-studio/"` appended — not `"assets/"`, which hid the README and the manifest from typos (corrected by S09) |
...
# (Comment as the Justfile has it since S05's sixth commit and S09; corrected by S09.)
assets-manifest:
...
- After `sync`, add `sync-python`: `uv sync --frozen` alone, with a comment saying it is
  the Python half of `sync` for a job that runs only Python recipes, needing no registry
  token and installing no `node_modules`. The `assets` job in CI runs it (§8; added by S09).
...
[tool.typos.files]
# Lockfiles carry hashes that look like typos; the imported assets carry base64
# and Blender's own vocabulary. The model's README and assets/manifest.json are
# not excluded: typos reads both, as markdownlint and lychee read the README.
extend-exclude = [
  "package-lock.json",
  "uv.lock",
  "assets/models/biscuit/model/",
  "assets/models/biscuit/poses/",
  "assets/models/biscuit/previews/",
  "assets/models/biscuit/qa/",
  "assets/models/biscuit/src/",
  "assets/models/biscuit/textures/",
  "assets/illustrations/",
  "static/pose-studio/",
]
...
| `static/pose-studio/viewer.html` | 27,561,668 | blob | served by Pages (S02's rewrite of three hrefs added 243 bytes; corrected by S09) |
...
| `assets/models/biscuit/textures/*.png` (36) | 10,040,783 total (corrected by S09 from the manifest) | blob | the largest is 2,288,330; sources, but small and rarely regenerated |
| `assets/models/biscuit/previews/**` (22) | 9,754,946 total (corrected by S09 from the manifest) | blob | the native renders and the studio screenshots; `pose-overview.jpg` is the one preview that lives under `static/pose-studio/previews/` instead, because the viewer links it |
| `assets/illustrations/good/*.png` (11) | 15,137,724 total (corrected by S09 from the manifest) | blob | imported by the gallery page |
...
Costs, stated so they are chosen rather than found: GitHub's free LFS tier is 10 GiB of
storage and 10 GiB of bandwidth a month, with metered billing beyond that (GitHub Docs,
"About storage and bandwidth usage", read 2026-09-23 by S02; corrected by S09 from the
1 GB and 1 GB first written here, §10); the three LFS objects are 32.3 MB, so a clone that
fetches them spends about 0.3% of a month's bandwidth. Every rebuild of the viewer adds
26 MB of ordinary history, accepted while rebuilds are rare and named in decision 0006; C03
is the exit.
...
- `check` (what `just check-assets` runs): walk both roots; refuse a file with no entry
  ...
  with `git check-attr
  filter <path>` (`lfs` ↔ `filter: lfs`); refuse a `source` that is not
  `<repository>@<commit>:<path>`, `rebuilt:<date>` or `studio` (S09); for every path whose
  attribute is `lfs` and which the index holds, read the index blob with `git cat-file` and
  refuse one that is not an LFS pointer, since a `.blend` added without git-lfs hashes to
  the recorded oid and `check-attr` cannot tell (S09); refuse `source_sha256` or `patched` present
...
- `write` (what `just assets-manifest` runs): recompute every entry from the worktree,
  ...
  whenever that record is present; then run `check` and exit with its status. It runs
  `self-test` first, as `check` does, and writes nothing if that fails (S09). It never
...
- `self-test`: build a temporary tree with one blob, one hand-written LFS pointer, one
  ...
  and refuses the fourth with `GPS`; put the pointer's path into the index as the file
  itself with `git hash-object --stdin` and `git update-index --cacheinfo`, which bypass
  the filter, and assert `check` refuses it naming the LFS pointer, then as the pointer
  and assert it passes; assert `source` passes in each of its three forms and `unknown`
  is refused (both S09); then, each generated in the temporary tree, assert a
...
reads), sync-python, check-assets. The token sits on the `just sync` step of `frontend`
and `documents` only; the `assets` job runs `sync-python` (§2.2) and carries none
(corrected by S09 from the S04 open point). No job carries a `paths` filter or a `name:`: a
skipped required check blocks the merge, and the job id is the check's name (T C03).
...
- `pillow==12.3.0` resolves under `uv lock` for Python 3.14 beside the tooling package,
  ...
  in the self-test is a JPEG. **S00.** Held: `uv lock` resolved `pillow==12.3.0` under
  CPython 3.14.3 beside the tooling package (S00 hand-back, "The Pillow pin").
- The prek `exclude` regex alone keeps editorconfig-checker, typos, lychee and
  ...
  **S02** (with the real files: `just lint` must pass with a 26 MB file in the tree).
  Held: partly settled on S00's empty tree (S00 open points), then every hook passed with
  the viewer and the GLB in the tree and no hook excluded beyond §2.1 (S02 hand-back,
  "Step 11").
- `vitest --coverage` with thresholds passes on a `src/lib/` holding only `brand.ts`,
  which `tests/brand.test.ts` imports. **S00.** Held: 100% on all four figures, with an
  empty per-file table (S00 hand-back, "Coverage over a one-file `src/lib/`").
- `svelte-check --fail-on-warnings` and `vite build` tolerate a 26 MB file under
  ...
  **S03.** Held: nothing printed about the file, and the viewer and the GLB copied byte
  for byte (S03 hand-back, "The four CONVENTIONS §10 claims").
- `@sveltejs/kit` accepts a route directory (`src/routes/model/`) and a static directory
  ...
  **S03.** Held: no collision warning, and both `/model/` and `/pose-studio/viewer.html`
  served (S03 hand-back, "The four CONVENTIONS §10 claims").
- The prerender crawler (`strict: true`) treats an `<a href>` into `static/` as an asset
  ...
  regardless; the claim is what happens without them. **S03.** Held: a scratch build with
  both attributes removed wrote `build/model/index.html` and exited 0, and the click with
  them in place left the app (S03 hand-back, "The four CONVENTIONS §10 claims").
...
- `actions/checkout` with `lfs: false` (the default) checks out the pointer text, and
  ...
  writes it, **S07** proves it on the first run. S04: actionlint accepts the option under
  that name (S04 open points, "`lfs: false` on `actions/checkout` v7.0.1"); the first run
  is S07's.
- GitHub's free LFS tier is 1 GB storage and 1 GB bandwidth a month. **S02** reads the
  ...
  Failed and corrected: the free tier is 10 GiB of storage and 10 GiB of bandwidth a
  month, metered beyond, read 2026-09-23; §3 says so since S09 (S02 hand-back, "The LFS
  quota figure").
...
- A Vite import of a PNG from outside `src/` but inside the repository root
  ...
  build, dev and jsdom without `server.fs.allow`. **S03.** Failed for `vite dev`, which
  answered `403` outside its allow list while build and jsdom were fine; the correction is
  `server.fs.allow: ['assets']` in `vite.config.ts`, committed by S03 (S03 hand-back, "The
  four CONVENTIONS §10 claims").
- `document.title` is reflected by a `<svelte:head><title>` under jsdom (T CONVENTIONS
  ...
  proves flaky). **S01.** Held: reflected, and the case passed on every run and was kept
  (S01 hand-back, "`document.title` under jsdom").
...
- **git-lfs is installed per machine, and `git lfs install --local` runs per clone.**
  `scripts/initialize.sh` runs the second. A contributor without it adds a real `.blend`
  as a blob; `git check-attr` cannot tell, since it reads `.gitattributes` and the bytes
  hash to the recorded oid, so `check-assets` reads the index blob of every LFS path and
  refuses one that is not a pointer, from the first run after `git add` (corrected by S09:
  S05 and S06 hand-backs, and the checker change S09 made).
...
- **The gallery page is heavy** (eleven PNG files, 15,141,024 bytes transferred once the
  page is scrolled to the end, measured by S03). Accepted for the first release;
  thumbnails are S10.
